...
| `interop/results/20260917T213457Z/` | the matrix run: `matrix.md`, `results.tsv`, `versions.txt` and all 96 per-run logs (the earlier runs `20260917T015709Z`, `20260917T102958Z` and `20260917T134954Z`, superseded, are kept beside it) |
...
Run directory: [`interop/results/20260917T213457Z/`](interop/results/20260917T213457Z/). Runner:
...
Implementations tested (from `versions.txt`): JS `b4a552e`, Python `b77b52a2`, Nim `2de86ac`,
...
This run repeats the matrix at the Nim branch head that follows vacp2p/nim-libp2p master being
merged into it; the earlier run `20260917T134954Z` covered the same four implementations before
that merge. The merge left the NoiseHFS sources byte-identical but renamed an `Opt` helper the
handshake calls, so the Nim head here carries the one-line fix that follows the rename.
